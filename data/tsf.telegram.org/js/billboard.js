...
extend(ChartInternal_ChartInternal.prototype, {
  ...
  updateXs: function updateXs() {
    var $$ = this;
    $$.data.targets.length && ($$.xs = [], $$.getMaxDataCountTarget($$.data.targets).values.forEach(function (v) {
      $$.xs[v.index] = v.x;
    }));
  },
  ...
  getTotalPerIndex: function getTotalPerIndex() {
    ...
    if ($$.isStackNormalized() && !sum && (sum = [], $$.data.targets.forEach(function (row) {
      row.values.forEach(function (v, i) {
        sum[i] || (sum[i] = 0), sum[i] += isNumber(v.value) ? v.value : 0;
      });
    })), $$.addCache("$totalPerIndex", sum.slice()), $$.hiddenTargetIds.length) {
      var hiddenSum = $$.api.data.values.call($$.api, $$.hiddenTargetIds, !1);
   ...
   * @private
   */
  getRatio: function getRatio(type, d, asPercent, totalCache) {
    var $$ = this,
        config = $$.config,
        api = $$.api,
        ratio = 0;
    if (d && (totalCache || api.data.shown.call(api).length)) if (ratio = d.ratio || d.value, type === "arc") {
      var dataValues = api.data.values.bind(api); // if has padAngle set, calculate rate based on value
      ...
      if (d.value === 0) return d.ratio = 0, d.ratio;

      var _total = totalCache === undefined ? this.getTotalPerIndex() : totalCache;
  ...
  getShapeOffset: function getShapeOffset(typeFilter, indices, isSub) {
    ...
    return function (d, idx) {
      var total,
          ...
          i = idx,
          x2 = +d.x;
      return isStackNormalized && (total = $$.getTotalPerIndex()), targets.forEach(function (t) {
        if (t.id !== d.id && indices[t.id] === indices[d.id]) {
          ...
          var rowValues = t.values;

          if (targetIds.indexOf(t.id) < targetIds.indexOf(d.id) && ((isUndefined(rowValues[i]) || +rowValues[i].x !== x2) && (i = -1, rowValues.forEach(function (v, j) {
            +v.x === x2 && (i = j);
          })), i in rowValues && rowValues[i].value * d.value >= 0)) {
            var v = rowValues[i],
                value = isStackNormalized ? $$.getRatio("index", v, !0, total) : v.value;
...
extend(ChartInternal_ChartInternal.prototype, {
  ...
  tooltipPosition: function tooltipPosition(dataToShow, tWidth, tHeight, element) {
    ...
    return right > chartRight && (left = left - tWidth - 50), top + tHeight > $$.currentHeight && (top -= tHeight + 30), top < 0 && (top = 0), left < 0 && (left = 0), {
      top: top,
      left: left
    };
  },
...
extend(ChartInternal_ChartInternal.prototype, {
  ...
  initBrush: function initBrush() {
    var $$ = this,
        ...
        isRotated = config.axis_rotated,
        firstUpdate = !0;
    $$.brush = isRotated ? Object(external_commonjs_d3_brush_commonjs2_d3_brush_amd_d3_brush_root_d3_["brushY"])() : Object(external_commonjs_d3_brush_commonjs2_d3_brush_amd_d3_brush_root_d3_["brushX"])(), $$.brush.handleSize(10);

    // set "brush" event
    var brushHandler = function () {
      firstUpdate || $$.redrawForBrush();
    };

    // set the brush extent
    $$.brush.on("start", function () {
      $$.inputType === "touch" && $$.hideTooltip(), brushHandler();
    }).on("brush end", brushHandler), $$.brush.update = function () {
      ...
      if (firstUpdate && $$.config.subchart_defzoom) {
        var extent = this.extent()(),
            domain = $$.config.subchart_defzoom(extent);
        domain && domain.length && ($$.brush.move.call(this, $$.brush.getSelection(), domain), $$.brushHandlesUpdate(), changed = !0);
      }

      return firstUpdate = !1, changed;
    };
  },
  ...
  initSubchart: function initSubchart() {
    ...
    var context = $$.context;
    context.style("visibility", visibility), context.append("g").attr("clip-path", clipPath).attr("class", config_classes.chart), $$.hasType("bar") && context.select(".".concat(config_classes.chart)).append("g").attr("class", config_classes.chartBars), context.select(".".concat(config_classes.chart)).append("g").attr("class", config_classes.chartLines), $$.axes.subx = context.append("g").attr("class", config_classes.axisX).attr("transform", $$.getTranslate("subx")).attr("clip-path", config.axis_rotated ? "" : $$.clipPathForXAxis).style("visibility", config.subchart_axis_x_show ? visibility : "hidden"), context.append("g") // .attr("clip-path", clipPath)
    .attr("class", config_classes.brush).call($$.brush);
    var brush = context.select(".".concat(config_classes.brush)),
        handleW = context.select(".".concat("handle--w")),
        handleE = context.select(".".concat("handle--e"));
    $$.brushHandleW = brush.insert("line", ":first-child").attr("class", "bb-brush-handle bb-brush-handle--w").attr("stroke-linecap", "round"), $$.brushHandleE = brush.insert("line", ":first-child").attr("class", "bb-brush-handle bb-brush-handle--e").attr("stroke-linecap", "round"), $$.brushHandlesUpdate = function () {
      if (handleE.style("display") !== "none") {
        var xW = parseInt(handleW.attr("x"), 10) + parseInt(handleW.attr("width") / 2, 10) + 1,
            xE = parseInt(handleE.attr("x"), 10) + parseInt(handleE.attr("width") / 2, 10) - 0,
            y1 = parseInt(handleW.attr("y"), 10) + 1,
            y2 = y1 + parseInt(handleW.attr("height"), 10) - 2;
        xW > xE && (xE = (xW + xE) / 2, xW = xE), $$.brushHandleW.attr("x1", xW).attr("x2", xW).attr("y1", y1).attr("y2", y2).style("display", ""), $$.brushHandleE.attr("x1", xE).attr("x2", xE).attr("y1", y1).attr("y2", y2).style("display", "");
      } else $$.brushHandleW.style("display", "none"), $$.brushHandleE.style("display", "none");
    };
  },
  ...
  redrawSubchart: function redrawSubchart(withSubchart, duration, shape) {
    ...
    })), $$.brushHandlesUpdate && $$.brushHandlesUpdate());
  },
